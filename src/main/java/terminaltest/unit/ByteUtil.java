...
public class ByteUtil {
    ...
    /* CCITT标准CRC16(1021)余数表 CRC16-CCITT ISO HDLC, ITU X.25, x16+x12+x5+1 多项式 高位在先时生成多项式 Gm=0x11021 低位在先时生成多项式，Gm=0x8408 本例采用高位在先 */
    ...
    public static int toCRC(int regInit, byte[] bytes) {
        ...
        for (int i = 0; i < bytes.length; i++) {
            crcReg = (crcReg >> 8) ^ CRC16_CCITT_TABLE[(crcReg ^ bytes[i]) & 0xff];
        }
        return crcReg;
    }
